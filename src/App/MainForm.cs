...
public sealed class MainForm : Form
{
    ...
    public MainForm()
    {
        Text = "Discord TTS — VB-CABLE Edition";
        Width = 920;
        Height = 650;
    ...
    private void InitializeAll()
    {
        ...
        {
            if (_output.Items[i] is MMDeviceSelection item &&
                item.Name.Contains("CABLE Input", StringComparison.OrdinalIgnoreCase))
            {
                cableIndex = i;
                break;
            }
        ...
        if (cableIndex >= 0)
        {
            ...
            _status.Text = "VB-CABLE detected. In Discord select CABLE Output as Input Device.";
        }
        else
        {
            if (_output.Items.Count > 0) _output.SelectedIndex = 0;
            _status.Text = "VB-CABLE not detected. Install VB-CABLE, then select CABLE Input here.";
        }
    ...
    private void RestartAudio()
    {
        ...
        try
        {
            ...
            _status.Text = output.Name.Contains("CABLE Input", StringComparison.OrdinalIgnoreCase)
                ? "Running. Discord Input Device = CABLE Output (VB-Audio Virtual Cable)."
    ...
    private void ApplyLevels() =>
        _audio.SetLevels(_micVol.Value / 100f, _ttsVol.Value / 100f, _passMic.Checked, _duck.Checked, 0.25f);
    ...
    private async Task SpeakAsync()
    {
        ...
        {
            using var synth = new SpeechSynthesizer();
            if (_voice.SelectedItem is string voiceName)
            ...
            _audio.QueueTtsPcm16(bytes);

            var durationMs = bytes.Length / 2.0 / AudioMixerEngine.SampleRate * 1000.0;
            _status.Text = "Speaking through VB-CABLE…";
            _text.Clear();
